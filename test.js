...
function steamProfile(id, msg, integer) {
  ...
  https.get("https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=" + process.env.STEAM_TOKEN + "&steamids=" + id, res => {
    ...
    res.on("end", () => {
      ...
      // Check if there is a document with the id, if it exists, do nothing, else push the data to firestore (needs tweeking)
      let idRef = db.collection('gameBot');
      let query = idRef.where('id', '==', id).get()
        .then(snapshot => {
          if (snapshot.empty) {
            console.log('No matching profile.');
            db.collection('gameBot').doc(Date.now().toString()).set(data)
          } else {
